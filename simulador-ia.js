...
if (!process.env.OPENAI_API_KEY?.trim()) {
  console.error("Falta OPENAI_API_KEY no arquivo .env.");
  process.exit(1);
}
...
async function executar() {
  console.log("\nALPS — SIMULADOR COM IA");
  console.log(`Modelo: ${modelo}`);
  console.log("Use pacientes fictícios. As mensagens serão enviadas à OpenAI.");
  console.log("Histórico em memória, sem gravação em arquivo.");
  console.log("Teste liberado em qualquer horário.");
  console.log("Comandos: /novo para reiniciar; /sair para encerrar.\n");

  while (true) {
    const texto = (await terminal.question("Paciente: ")).trim();
    ...
    if (texto.length > 3000) {
      console.log("\nPara este teste, envie até 3.000 caracteres.\n");
      continue;
    }

    if (historico.length >= 40) {
      console.log("\nLimite de 20 trocas neste teste. Use /novo.\n");
      continue;
    }
    ...
    try {
      ...
      const mensagem = resposta.output_text?.trim();

      if (resposta.status !== "completed" || !mensagem) {
      ...
      console.log(
        ...
        `saída ${resposta.usage?.output_tokens ?? "?"}]\n`
      );
    } catch (erro) {
      // Não imprime a chave nem o objeto completo do erro.
      if (erro.status === 401) {
        console.log("\nA API recusou a credencial. Confira a chave no .env.\n");
      } else if (erro.status === 429) {
        console.log("\nLimite da API atingido ou saldo/cota indisponível.\n");
      } else if (erro.status === 403 || erro.status === 404) {
        console.log("\nConfira o acesso do projeto ao modelo configurado.\n");
      } else {
        console.log(
          `\nFalha na chamada da API. Status: ${erro.status || "conexão"}.\n`
        );
      }
    }
  }
